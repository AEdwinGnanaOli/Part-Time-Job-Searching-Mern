...
module.exports.VendorSignUp = async (req, res, next) => {
    try {
        ...
        res.cookie('token', token);
        return res.json({ message: "User Sign Successfully", Status: 'Success', vendor });
        next()
    } catch (err) {
        console.log(err)
    }
...
module.exports.ProductRegister = async (req, res) => {
    try {
      ...
      if (!shopname || !email || !shopmobilenumber || !shopaddress) {
      ...
      const imagePath = req.file?.path || null; // The Cloudinary URL will be stored here
      console.log("Uploaded file path (Cloudinary URL):", imagePath);
      ...
      const product = await ProductModel.create({
        ...
        work,
        price,
        image: imagePath, // Save the Cloudinary URL
        role,
      });
