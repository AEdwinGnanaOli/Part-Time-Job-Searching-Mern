...
function UserDetails() {
  ...
  const handleUserUpdate = (id) => {
    navigate(
      "/admin/user/update",
      sessionStorage.setItem("adminuupdateid", id)
    );
  };
  const colums = [
    {
      ...
      cell: (row) => (
        <div className="admin-user-btns">
          <Button
            ...
            variant="success"
            onClick={(e) => handleUserUpdate(row._id)}
          >
            <i>
  ...
  useEffect(() => {
    const newdata = userdata.filter((res) => {
      return (
        res.name.toLowerCase().includes(qurey.toLowerCase().trim()) ||
        res.email.toLowerCase().includes(qurey.toLowerCase().trim()) ||
        res.phone.toLowerCase().includes(qurey.toLowerCase().trim()) ||
        res.role.toLowerCase().includes(qurey.toLowerCase().trim())
      );
    });
